...
io.on("connection", async (socket) => {
    ...
    socket.on("message", async (response: MessageDataFields) => {
        try {
            ...
            console.log("[server] EXCEPTION THROWN - shutting down socket");
            socket.emit("message", constructFailureMessage("Something went wrong. Trying again later"));
            socket.disconnect();
        }
    });
...
["exit", `SIGINT`].forEach((eventType:any) => {
